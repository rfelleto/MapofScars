...
function GetDifficultyColor(level)
    ...
    -- trivial (5 or more levels below player)
    if level <= playerLevel - 5 then
        return { r = 0.6, g = 0.6, b = 0.6 } -- grey
    end

    -- otherwise use Blizzard's normal system
    return original_GetDifficultyColor(level)
end
...
-- check if the target is YOUR pet
if UnitExists("pet") and UnitIsUnit("target","pet") then
    ...
    if happiness == 1 then
        reaction = "looks uneasy and agressive."
    elseif happiness == 2 then
        reaction = "listens to your commands with mild interest."
    elseif happiness == 3 then
        reaction = "regards you with loyalty and enthusiasm."
    else
        -- warlock pets
        reaction = "awaits your command with unnatural obedience."
    end

    -- wound descriptions
    if health >= 0.75 then
        wounds = "It appears healthy and ready for battle."
    elseif health >= 0.50 then
        wounds = "It bears a few scratches but remains eager."
    elseif health >= 0.25 then
        wounds = "It looks wounded and tired."
    else
        wounds = "It is on the brink of death."
    end

    local name = UnitName("pet")

    DEFAULT_CHAT_FRAME:AddMessage(
        name.." "..reaction.." "..wounds,
        1, 0.82, 0
    )
...
petFrame:SetScript("OnUpdate", function()
    ...
    -- trigger warning under 40%
    if health < 0.35 and not petWarned then
        local name = UnitName("pet")
    ...
    -- reset if pet recovers above 50%
    if health > 0.5 then
        petWarned = false
    end
end)
...
if targetLevel == -1 then
    DEFAULT_CHAT_FRAME:AddMessage(
        targetName.." "..reactionText.." What would you like your tombstone to say?",
        1, 0, 0
    )
    return
end
